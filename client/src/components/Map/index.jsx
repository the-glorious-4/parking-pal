...
//   console.log("Current user from state: " + state.currentUser);
  ...
  return (
    <div className="mapBody">
      <h1 className="mapTitle">
        Parking-Pal <span role="img">🚗</span>
      </h1>

      {props.findMeBtn ? (
        <div className="findMeBtn">
          <FindMeBtn />
        </div>
      ) : null}
      ...
      <GoogleMap
        ...
        zoom={15}
        center={
          state.mapLocation ? state.mapLocation : { lat: 37.774, lng: -122.419 }
        }
        options={options}
        // onLoad={onLoad}
      >
          ...
          markers.map((marker) => (
            <Marker
              ...
              onClick={() => {
                ...
                console.log("selectedInventory from state: " + JSON.stringify(state.selectedInventory));

                dispatch({
                  type: UPDATE_MAP_LOCATION,
                  location: {
                    lat: parseFloat(marker.parkingPlace.latLng[0]),
                    lng: parseFloat(marker.parkingPlace.latLng[1]),
                  },
                });
              }}
              position={{
        ...
        {state.selectedInventory ? (
          <InfoWindow
            position={{
              lat: parseFloat(state.selectedInventory.parkingPlace.latLng[0]),
              lng: parseFloat(state.selectedInventory.parkingPlace.latLng[1]),
            }}
            onCloseClick={() => {
              dispatch({
                type: UPDATE_SELECTED_INVENTORY,
          ...
          >
            <div className="mapInfoWindow">
              <h3 style={{ textAlign: "center" }}>
                ${state.selectedInventory.price}/day
              </h3>
              <p>
                {state.selectedInventory.parkingPlace.street},{" "}
                {state.selectedInventory.parkingPlace.city}
                <br />
                {state.selectedInventory.parkingPlace.isCoveredParking
                  ? "Indoor Parking"
                  : "Outdoor Parking"}
              </p>
              <Link to="checkout">
                <button
                  ...
                  onClick={() =>
                    console.log("CLICKCED! " + JSON.stringify(state))
                  }
                >
                  Reserve
                </button>
